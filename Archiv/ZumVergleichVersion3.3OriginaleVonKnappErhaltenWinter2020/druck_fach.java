...
public class druck_fach extends Frame {
 ...
 int counting = 0;
  ...
  try {//Anfang try 0
  FileWriter files = new FileWriter("C:/db_sba/Fachlisten.doc");
    try { //try 1
      String url = "jdbc:odbc:sba"+sjstr;
      Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
      Connection con = DriverManager.getConnection(url,"lehrer","jon96as12");
      Statement stmt = con.createStatement();
      ResultSet fach = stmt.executeQuery("SELECT * FROM faecher ORDER BY Fach");
      ...
      while (fach.next()){//Beginn while 0
        ...
        try { //Anfang try 2
          ...
          for (j=0;j<klcb_row;j++) {
            ...
            ResultSet rs = stmts.executeQuery("SELECT * FROM "+klassecb+" WHERE Fach like '%"+fachtmp+"' ORDER BY Fach");
            while (rs.next()) {
              ...
              werttmp[d] = rs.getDouble(8);
                ...
                double wert1 = (werttmp[d]*10)-((int)(werttmp[d]*10));
                if (wert1 < 0.5) werttmp[d] = werttmp[d]*10;
                else werttmp[d] = (werttmp[d]*10)+1;
          ...
          if (wert1 < 0.5) wertges = wertges*10;
          else wertges = (wertges*10)+1;
          int werttmps = (int)wertges;
          wertges = (double)werttmps/10;

          a=0;b=0;c=0;d=0;l=0;v=0;x=0;

          files.write("LISTE ZUR KONTROLLE der eingegebenen Bücher vom Fach "+fachtmp+" zur Schulbuchaktion "+date+" ("+(char)169+" A.Knapp)\n");
          files.write(text+"\n");
          ...
          for(a=0;a<count_buch;a++) {
            ...
            files.write(nrtmp[a] + TabNr + titeltmp[b] + Tab1 + preistmp[c] + "\t" + werttmp[d] + "\t" + wiedertmp[v] + "\t" +lehrer[l] + "\t" + klassecbtmp[x] + "\n");
            b++;c++;d++;l++;v++;x++;
          }
  ...
  JOptionPane.showMessageDialog(null,"Bücher für " + counting + " Fächer auf C:/db_sba/Fachlisten.doc geschrieben !");
  }
}