...
public class limits extends Frame {
  ...
  public void jbInit2() {
   ...
   //Das Fenster zentrieren
    ...
    limitsfrm.setLocation((screenSize.width - frameSize.width) / 2, (screenSize.height - frameSize.height) / 2);
  ...
  if (usint == 0 || ussbxint == 0 || usrelint == 0 || usrelsbxint == 0 || osint == 0 || ossbxint == 0 || osrelint == 0 || osrelsbxint == 0) JOptionPane.showMessageDialog(null,"Keine Eingabe !");
  ...
  try  {
      ...
      int response = JOptionPane.showConfirmDialog(null,"Alte Limits überschreiben ?","Limits",JOptionPane.YES_NO_OPTION);
      if (response == 0) {
      String sql = "DELETE FROM limits_gesamt";
      stmt.execute(sql);
      sql = "INSERT INTO limits_gesamt (Unterstufe,Unterstufe_SBX,Unterstufe_Religion,Unterstufe_Religion_SBX,Oberstufe,Oberstufe_SBX,Oberstufe_Religion,Oberstufe_Religion_SBX) VALUES ("+usdouble+","+ussbxdouble+","+usreldouble+","+usrelsbxdouble+","+osdouble+","+ossbxdouble+","+osreldouble+","+osrelsbxdouble+")";
      stmt.executeUpdate(sql);
      JOptionPane.showMessageDialog(null,"Die neuen Limits wurden angelegt !");
  ...
  try  {
      ...
      stmt.execute(sql);
      sql = "INSERT INTO limits (Unterstufe,Unterstufe_SBX,Unterstufe_Religion,Unterstufe_Religion_SBX,Oberstufe,Oberstufe_SBX,Oberstufe_Religion,Oberstufe_Religion_SBX) VALUES ("+usdouble+","+ussbxdouble+","+usreldouble+","+usrelsbxdouble+","+osdouble+","+ossbxdouble+","+osreldouble+","+osrelsbxdouble+")";
      stmt.executeUpdate(sql);
      JOptionPane.showMessageDialog(null,"Die neuen Limits wurden angelegt !");
      limitsfrm.setVisible(false);
      con.close();
