...
double tmp_summe = 0, tmp_gesamt = 0, sbx_gesamt = 0, rel_gesamt = 0, uew_gesamt = 0, gesamt = 0, gesamt_ohne = 0;
double sbx, rel, uew;
int i = 0, count = 0;
...
//Beachte: klassecb wurde in dieser Klasse mehrmals ersetzt!
  try {
    ...
    ResultSet rs1 = stmt.executeQuery("SELECT * FROM klassen ORDER BY Klasse");
    while (rs1.next()) {
...
//Ende des Einschub-NEU
  ...
  try {
    ...
    Connection con = DriverManager.getConnection(url,"lehrer","jon96as12");
    Statement stmt = con.createStatement();
    ResultSet rs = stmt.executeQuery("SELECT * FROM "+klassenanzahl[j]);
    while (rs.next()) {
    ...
    //ABFRAGE und BERECHNUNG für SBX oder Religion oder uew sonst allgemein
    boolean fach_sbx = fach_re.startsWith("X");
    boolean fach_rel = fach_re.startsWith("R");
    boolean fach_relX = fach_re.startsWith("X-R");
    if (fach_relX == true) {fach_sbx = false; fach_rel = true; }
    boolean fach_relXK = fach_re.startsWith("XK-R");
    if (fach_relXK == true) {fach_sbx = false; fach_rel = true; }
    boolean fach_uew = titel_uew.startsWith("UeW");
    if (fach_sbx == true) sbx = sbx+tmp2[i];
    else if (fach_rel == true || fach_relX == true || fach_relXK == true) rel = rel+tmp2[i];
    else if (fach_uew == true) uew = uew+tmp2[i];
    else tmp_summe = tmp_summe+tmp2[i];

    /*boolean fach_sbx = false;
    boolean fach_rel = false;
    boolean teste_sbx = fach_re.startsWith("X");
    if (teste_sbx == true) {
      boolean teste_rel = fach_re.startsWith("X-R");
      boolean teste_rel_XKR = fach_re.startsWith("XK-R");
    if (teste_rel == true || teste_rel_XKR == true) {
        fach_rel = true;
        }
      else {
        fach_sbx = true;
    ...
    boolean fach_uew = titel_uew.startsWith("UeW");
    if (fach_sbx == true) sbx = sbx+tmp2[i];
    else if (fach_rel == true) rel = rel+tmp2[i];
    else if (fach_uew == true) uew = uew+tmp2[i];
    else tmp_summe = tmp_summe+tmp2[i];*/
...
/*    for (i=0;i<count;i++) {
    sql = "UPDATE "+klassecb+" SET Wert = "+tmp2[i]+" WHERE Nr = '"+nrre[i]+"' AND Lehrer = '"+lehrer_tmp[i]+"'";
    ...
    gesamt_ohne = gesamt_ohne+tmp_summe; //Gesamtsumme Allgemein ohne SBX
    gesamt = gesamt +tmp_gesamt; //Gesamtsumme Allgemein und SBX
    ...
    sql = "INSERT INTO Kl_Summe (Klasse, Summe_SBX, Summe_ohneSBX, SBX, Religion, UeW, Gesamt, Gesamt_ohneSBX, Gesamt_SBX, Gesamt_Religion, Gesamt_UeW) values ('"+klassenanzahl[j]+"', "+tmp_gesamt+", "+tmp_summe+", "+sbx+","+rel+","+uew+","+gesamt+","+gesamt_ohne+","+sbx_gesamt+","+rel_gesamt+","+uew_gesamt+")";
    stmt.executeUpdate(sql);
    stmt.close();
    con.close();
    }
    ...
    JOptionPane.showMessageDialog(null,"Berechnung für Klassen erfolgreich durchgeführt !");
    } // Ende if Schleife
    else {
