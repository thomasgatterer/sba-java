...
public class druck_buch extends Frame {
  ...
  int j, i, anzahl=0, buch_ges=0; int[] anzahlin=new int[100];
  int buch_anzahl=0;
  String[] titelin = new String[500]; String[] fachin = new String[500]; String[] nrin = new String[500];
  double[] preisin = new double[500]; double[] wertin = new double[500];
  String sql="";
  ...
  for (j=0;j<klcb_row;j++) {
      ...
      while(rs.next()) {
        ...
        anzahl = i;
        for (i=0;i<anzahl;i++) {
          sql = "INSERT INTO Buecher_gesamt (Nr,Titel,Preis,Fach,Anzahl,Wert,Klasse,Stufe)" + " values('"+nrin[i]+"','"+titelin[i]+"',"+preisin[i]+",'"+fachin[i]+"','"+anzahlin[i]+"',"+wertin[i]+",'"+klassestr+"','"+klassestr.charAt(0)+"')";
  ...
  try {//try 0
    FileWriter klassenfile = new FileWriter("C:/db_sba/Buchlisten.doc");
    ...
    try { // try 1
      ...
      Connection con = DriverManager.getConnection(url,"lehrer","jon96as12");
      Statement stmt = con.createStatement();
      ResultSet rs = stmt.executeQuery("SELECT * FROM Buecher_gesamt ORDER BY Nr");

          klassenfile.write("LISTE FÜR DEN Buchhändler ZUR AUSGABE DER BESTELLTEN BÜCHER "+date+"\n\n");
          ...
          while (rs.next()) {
            ...
            if (z==0) {tmp_anz=f[0];tmp_wert=g[0];}
            if(z>0) {
            ...
            if (schreibe==true) {
              double wert1 = (tmp_wert*1000)-((int)(tmp_wert*1000));
                ...
                int cl = c[z-1].length();
                if (cl > 35) c[z-1] = c[z-1].substring(0,35);
                cl = c[z-1].length();
                ...
                klassenfile.write(b_tmp[z-1] + TabNr + c[z-1] + Tab1 + d[z-1] + Tab2 + e[z-1] + Tab3 + tmp_anz +"\t"+ tmp_wert + "\n");
                ...
                if (z==buch_ges-1 && letztes_buch == true) {
                    ...
                    cl = c[z].length();
                    if (cl > 35) c[z] = c[z].substring(0,35);
                    cl = c[z].length();
                    cl = 35-cl; if (cl >= 30) Tab1="\t\t\t\t\t\t"; else if (cl >= 24 && cl < 30) Tab1="\t\t\t\t\t"; else if (cl >= 18 && cl < 24) Tab1="\t\t\t\t"; else if (cl >= 12 && cl < 18) Tab1="\t\t\t"; else if (cl >= 6 && cl < 12) Tab1 = "\t\t"; else if (cl < 6) Tab1 = "\t";
                    index = 0;
                    ...
                    if (letztes_buch==true) klassenfile.write(b_tmp[z] + TabNr + c[z] + Tab1 + d[z] + Tab2 + e[z] + Tab3 + f[z] +"\t"+ g[z] + "\n");
                    buch_anzahl++;
                ...
                try {
                  ...
                  sql = "INSERT INTO Schulstufe (Nr,Titel,Preis,Fach,Anzahl,Wert,Klasse) values('"+b_tmp[z-1]+"','"+ c[z-1] +"',"+ d[z-1] +",'"+ e[z-1] +"',"+ tmp_anz +","+ tmp_wert +",'"+ (char)h[z-1].charAt(0) +"')";
                  stmt.executeUpdate(sql);
                  if (z==buch_ges-1 && letztes_buch == true) {
                    sql = "INSERT INTO Schulstufe (Nr,Titel,Preis,Fach,Anzahl,Wert,Klasse) values('"+b_tmp[z]+"','"+ c[z] +"',"+ d[z] +",'"+ e[z] +"',"+ f[z] +","+ g[z] +",'"+ (char)h[z].charAt(0) +"')";
                    stmt.executeUpdate(sql);
                    }
                  }
  ...
  JOptionPane.showMessageDialog(null,"Es wurden "+buch_ges+" Bücher eingegeben auf C:/db_sba/Buchliste.doc geschrieben");
