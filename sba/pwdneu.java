...
public class pwdneu extends Frame {
	...
	public void jbInit1() {
		...
		pwfrm.setLocation((screenSize.width - frameSize.width) / 2,
			...
			if (pwNEUstr_length == 5) {
				...
				if (pwNEUBESTstr_length == 5) {
					...
					if (testpw == 0) {
						...
						char a = pwNEUstr.charAt(0);
						int aint = (byte) a;
						char b = pwNEUstr.charAt(1);
						int bint = (byte) b;
						char c = pwNEUstr.charAt(2);
						int cint = (byte) c;
						char d = pwNEUstr.charAt(3);
						int dint = (byte) d;
						char e = pwNEUstr.charAt(4);
						int eint = (byte) e;
						...
						if ((aint < 58 && aint > 47)
							...
							gueltig = false;
						if (((bint < 58 && bint > 47)
								|| (bint < 91 && bint > 64) || (bint < 123 && bint > 96))
								&& gueltig == true)
							gueltig = true;
						else
							gueltig = false;
						if (((cint < 58 && cint > 47)
								|| (cint < 91 && cint > 64) || (cint < 123 && cint > 96))
								&& gueltig == true)
							gueltig = true;
						else
							gueltig = false;
						if (((dint < 58 && dint > 47)
								|| (dint < 91 && dint > 64) || (dint < 123 && dint > 96))
								&& gueltig == true)
							gueltig = true;
						else
							gueltig = false;
						if (((eint < 58 && eint > 47)
								|| (eint < 91 && eint > 64) || (eint < 123 && eint > 96))
								&& gueltig == true)
							gueltig = true;
						else
							gueltig = false;
						...
						if (gueltig == true && pwneuTest == 0) {
							...
							try {
								...
								ResultSet test = stmt
										.executeQuery("SELECT * FROM lehrer WHERE Lehrer = '"
												+ lehrer_neu + "'");
								while (test.next()) {
								...
								if (gefunden == true) {
									sql = "UPDATE lehrer SET Password = '"
											+ pwNEUstr + "' WHERE Lehrer = '"
											+ lehrer_neu + "'";
									...
									JOptionPane
											.showMessageDialog(null,
													"Password wurde erfolgreich geändert !");
									pwfrm.setVisible(false);
								} else
									JOptionPane
											.showMessageDialog(null,
													"Benutzername und altes Password stimmen nicht überein !");
					...
					} // Ende if-Abfrage für Übereinstimmung der neuen
						// Passwärter
					else
						JOptionPane
								.showMessageDialog(
										null,
										"Das neu eingegebene Password stimmt nicht mit dem bestätigten neuen Password überein !");
				} // Ende if-Abfrage für Eingabe neues Password
				else
					JOptionPane
							.showMessageDialog(null,
									"Sie müssen für das neue Password genau 5 Zeichen eingeben !");
			}// Ende if-Abfrage für Anzahl der Zeichen
			else
				JOptionPane
						.showMessageDialog(null,
								"Für das neue Password bitte genau 5 Zeichen eingeben !");
		} // Ende if-Abfrage für Eingabe altes Password
		else
			JOptionPane.showMessageDialog(null,
					"Sie müssen das alte Password eingeben !");
	}
