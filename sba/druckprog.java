	...
	public int print(Graphics g, PageFormat pf, int page)
		...
		while (tokens > 0) {
			...
			for (tabs = 0; tabs < tabs_anzahl; tabs++) {
				...
				langZeiletmp = druckZeile.length();
				System.out.println(langZeile);
				langZeile = langZeile + langZeiletmp;
				int laenge = druckZeile.length();
				if (seitendruck == true) {
					if (tabs == 0)
						g.drawString(druckZeile, xpos, ypos);
					if (tabs == 1)
						g.drawString(druckZeile, xpos + 80, ypos);
					if (tabs == 2)
						g.drawString(druckZeile, xpos + 160, ypos);
					if (tabs == 3)
						g.drawString(druckZeile, xpos + 240, ypos);
					if (tabs == 4)
						g.drawString(druckZeile, xpos + 320, ypos);
				} else if (tabs == 0)
					g.drawString(druckZeile, xpos, ypos);
				else if (tabs == 1)
					g.drawString(druckZeile, xpos + 60, ypos);
				else if (tabs == 2)
					g.drawString(druckZeile, xpos + 260, ypos);
				else if (tabs == 3)
					g.drawString(druckZeile, xpos + 300, ypos);
				else if (tabs == 4)
					g.drawString(druckZeile, xpos + 330, ypos);
				else if (tabs == 5)
					g.drawString(druckZeile, xpos + 360, ypos);
				else if (tabs == 6)
					g.drawString(druckZeile, xpos + 400, ypos);
				else if (tabs == 7)
					g.drawString(druckZeile, xpos + 440, ypos);
			}
			ypos = ypos + 10;
			xpos = xtmp;
			tokens--;
		}
		g.drawString("Ausdruck zur Schulbuchaktion " + (char) 169
				+ " by A.Knapp", xpos, ymax);
