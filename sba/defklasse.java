...
public class defklasse extends Frame {
	...
	public void jbInit2() {
		...
		klassefrm.setLocation((screenSize.width - frameSize.width) / 2,
		...
		if (testzahl == 0)
			...
			try {
				...
				if (abbruch == true)
					...
					String sql = "INSERT INTO klassen (Klasse) VALUES ('"
							+ klassestr + "')";
					stmt.executeUpdate(sql);
					sql = "CREATE TABLE "
							+ klassestr
							+ " (ID INTEGER, Nr TEXT, Titel TEXT, Preis DOUBLE, Fach TEXT, Anzahl INTEGER, Lehrer TEXT, Wert DOUBLE, W INTEGER, PRIMARY KEY (ID))";
					stmt.executeUpdate(sql);
					JOptionPane.showMessageDialog(null, "Klasse " + klassestr
							+ " wurde angelegt !");
				}
				con.close();
				klassetxt.setText("");
		...
		if (testzahl == 0)
			...
			try {
				...
				if (abbruch == true)
					...
					String sql = "DELETE FROM klassen WHERE Klasse = '"
							+ klassestr + "'";
					stmt.executeUpdate(sql);
