...
public class pwd extends Frame {
	...
	public pwd(String sjstr) {
		...
		try {
			...
			for (i = 0; i < anz; i++) {
				...
				if (b < 66)
					...
					b--;
				if (tmp == 0)
					lehpwd[i] = "" + (char) x + (char) y + (char) z + (char) a
							+ (char) b;
				if (tmp == 1)
					lehpwd[i] = "" + (char) y + (char) a + (char) x + (char) z
							+ (char) b;
				if (tmp == 2)
					lehpwd[i] = "" + (char) z + (char) b + (char) a + (char) x
							+ (char) y;
				if (tmp == 3)
					lehpwd[i] = "" + (char) x + (char) a + (char) b + (char) z
							+ (char) y;
				if (tmp == 4)
					lehpwd[i] = "" + (char) b + (char) a + (char) z + (char) y
							+ (char) x;
				if (tmp == 5)
					lehpwd[i] = "" + (char) a + (char) x + (char) y + (char) b
							+ (char) z;
				if (tmp == 6)
					lehpwd[i] = "" + (char) b + (char) x + (char) y + (char) z
							+ (char) b;
				j++;
				tmp--;
			}
			for (i = 0; i < anz; i++) {
				sql = "UPDATE lehrer SET ID = " + id[i] + ",Password = '"
						+ lehpwd[i] + "' WHERE Lehrer ='" + leh[i] + "'";
		...
		JOptionPane.showMessageDialog(null,
				"Aktion: Password Neuvergabe beendet");
	}
}
